...
public class YmlParser {
    public Scene parse(InputStream is) {
        ...
        for (Object o : os) {
            ...
            if (oh.containsKey("camera")) {
                ...
                double p = 180 + extractNumber(orientationInfoMap, "p");
                ...
                camera = new Camera(extractPosition(positionInfoMap), h, p, r, fovX, fovY);
                ...
                materialsInfo.put(materialId, new Material(ambientColor, diffuseColor, specularColor, specularPower));
            } else if (oh.containsKey("directional_light")) {
                HashMap directionalLightInfo = (HashMap)oh.get("directional_light");

                Vector direction = extractPosition((HashMap)directionalLightInfo.get("direction")).normal();

                direction = direction.mul(Matrix.rotationX(Math.PI / 2));
                ...
                Vector position = extractPosition((HashMap)pointLightInfo.get("position"));
            ...
            if (o instanceof String) {
                ...
                if (ho.containsKey("lcs")) {
                    ...
                    if (lcs.containsKey("x")) {
                        tx = extractNumber(lcs, "x");
                    }

                    if (lcs.containsKey("y")) {
                        ty = extractNumber(lcs, "y");
                    }

                    if (lcs.containsKey("z")) {
                        tz = extractNumber(lcs, "z");
                    }
                    ...
                    if (lcs.containsKey("h")) {
                        rx = Math.PI * extractNumber(lcs, "h") / 180;
                    }
                    ...
                    if (lcs.containsKey("r")) {
                        rz = Math.PI * extractNumber(lcs, "r") / 180;
                    }
                    ...
                    try {
                        so = new ObjModelLoader().load(new File(model));
    ...
    private Vector extractPosition(HashMap map) {
        return new Vector(extractNumber(map, "x"), extractNumber(map, "y"), extractNumber(map, "z"));
    }
